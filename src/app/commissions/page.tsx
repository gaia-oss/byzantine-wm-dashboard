...
import { fetchCommissionBreakdown, fetchDashboardKPIs } from "@/lib/services";
...
export default function CommissionsPage() {
  ...
  const { data, loading } = useAsyncData(
    () =>
      Promise.all([fetchDashboardKPIs(), fetchCommissionBreakdown()]).then(
        ([kpis, breakdown]) => ({
          kpis,
          breakdown,
        })
  ...
  const earningsCards = [
    {
      ...
      value: formatCurrency(kpis?.lifetimeEarnings || 47575),
      subtitle: "All-time earnings",
    },
    {
      title: "Earned This Year",
      value: formatCurrency(kpis?.yearToDateEarnings || 36825),
      subtitle: "Year-to-date",
    },
    {
      title: "This Quarter (Running)",
      value: formatCurrency(kpis?.estimatedQuarterlyCommission || 18275),
      subtitle: "Q1 2026 estimate",
    },
    {
      title: "Projected Next Quarter",
      value: formatCurrency(kpis?.projectedNextQuarter || 22850),
                ...
                {breakdown.map((row) => (
                  <tr key={row.clientId}>
                    ...
                    <td className="text-[#6B5A70]">
                      {[28, 36, 40].includes(row.bps) ? "Year 1" : "Year 2+"}
                    </td>
                    <td className="font-semibold text-[#702963]">
                      {formatBps(row.bps)}
                    </td>
                    <td className="font-semibold">
                      {formatCurrency(row.quarterlyEarning)}
                    </td>
                    <td className="font-semibold text-[#16A34A]">
                      {formatCurrency(row.annualizedEarning)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        >
              ...
              <tbody>
                {COMMISSION_TIERS.map((tier) => (
                  <tr key={tier.label}>
                    ...
                    <td className="text-[#16A34A] font-semibold">
                      {formatBps(tier.year1Bps)}
                    </td>
                    <td className="text-[#F59E0B] font-semibold">
                      {formatBps(tier.year2Bps)}
                    </td>
                    <td className="text-[#2563EB] font-semibold">
                      {formatBps(tier.year3Bps)}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="bg-[#F9F0F7] border border-[#E0B3D9] rounded-lg p-4 space-y-2">
            <p className="text-sm text-[#6B5A70]">
              <span className="font-semibold">Commission calculated</span> on
              average daily AUM over the quarter
            </p>
            <p className="text-sm text-[#6B5A70]">
              <span className="font-semibold">Deposits in last 30 days</span> of
              quarter are pro-rated
            </p>
            <p className="text-sm text-[#6B5A70]">
              <span className="font-semibold">10M+ clients:</span> auto-renewed,
              no time limit
            </p>
            <p className="text-sm text-[#6B5A70]">
              <span className="font-semibold">Below 10M:</span> 36-month term,
              renewable
            </p>
            <p className="text-sm text-[#6B5A70]">
              <span className="font-semibold">Payment:</span> quarterly, via
              bank transfer or Byzantine Prime account
            </p>
          </div>
        </motion.div>
