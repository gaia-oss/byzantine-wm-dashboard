...
import { fetchClientFunnel, fetchClients } from "@/lib/services";
...
export default function ClientsPage() {
  ...
  const { data, loading } = useAsyncData(
    () =>
      Promise.all([fetchClients(), fetchClientFunnel()]).then(
        ([clients, funnel]) => {
          const withCommission: ClientWithCommission[] = clients.map(
            (client) => {
  ...
  const handleFunnelChange = useCallback(
    (stage: string | null) => setFunnelFilter(stage),
    []
  );
    ...
    result.sort((a, b) => {
      const aVal =
        sortField === "commission"
          ? a.quarterlyCommission
          : sortField === "tier"
            ? a.commissionTier
            : (a as any)[sortField];
      const bVal =
        sortField === "commission"
          ? b.quarterlyCommission
          : sortField === "tier"
            ? b.commissionTier
            : (b as any)[sortField];
