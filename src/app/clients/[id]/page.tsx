...
export default function ClientDetailPage({ params }: ClientDetailPageProps) {
  const { t } = useI18n();
  const [clientOverride, setClientOverride] = useState<ClientDetail | null>(null);

  const { data: fetchedClient, loading, error } = useAsyncData(
  ...
  if (error || !client) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        <Link href="/clients" className="inline-flex items-center gap-2 text-[#702963] hover:text-[#5A1F4F] font-medium">
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <Link href="/clients" className="inline-flex items-center gap-2 text-[#702963] hover:text-[#5A1F4F] font-medium transition-colors">
        <ArrowLeft size={18} /> Back to clients
      </Link>
      ...
      {/* KPI Cards */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard title="Commission Earned" value={formatCurrency(client.commissionHistory?.reduce((s, p) => s + p.amount, 0) || 0)} />
        <KpiCard title="Current Yield" value={`${(((client.managementFee || 0) / 10000) * 100).toFixed(2)}%`} />
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <FeeEditor currentFee={client.managementFee} clientAum={client.aum} onSave={handleSaveFee} />
      </motion.div>
