...
import { TrendingUp, Users, Coins, Calendar, ArrowUpDown } from 'lucide-react';
...
export default function DashboardHome() {
  ...
  const kpiCards = kpis
    ? [
        { title: t.overview.totalAum, value: formatCurrency(kpis.totalAum), change: kpis.totalAumChange, trend: 'up' as const, icon: <TrendingUp size={20} /> },
        { title: t.overview.activeClients, value: `${kpis.activeClients} / ${kpis.totalClients}`, subtitle: 'of total clients', icon: <Users size={20} /> },
        { title: t.overview.estimatedCommission, value: formatCurrency(kpis.estimatedQuarterlyCommission), subtitle: 'Running estimate', icon: <Coins size={20} /> },
        { title: 'Next Payout', value: getNextPayoutDate(), subtitle: 'Date countdown', icon: <Calendar size={20} /> },
        {
          title: 'Net Deposits Q1',
          value: formatCurrency(kpis.totalDepositsThisMonth - kpis.totalWithdrawalsThisMonth),
          change: (kpis.totalDepositsThisMonth - kpis.totalWithdrawalsThisMonth) > 0 ? 5.2 : -2.1,
          trend: ((kpis.totalDepositsThisMonth - kpis.totalWithdrawalsThisMonth) > 0 ? 'up' : 'down') as 'up' | 'down',
