  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-40 p-2 rounded-lg bg-white border border-[var(--color-border)] text-[#702963] md:hidden"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-screen w-[260px] bg-white border-r border-[var(--color-border)] flex flex-col z-50 transition-transform duration-300 ease-in-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0`}
      >
        {/* Logo + Mobile close button */}
        <div className="px-6 py-8 border-b border-[var(--color-border)] flex items-center justify-between">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="p-1 rounded-lg text-[#6B5A70] hover:bg-[#F3EFF5] md:hidden"
