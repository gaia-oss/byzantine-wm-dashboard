...
function SortIndicator({ field, active, direction }: { field: SortField; active: SortField; direction: SortDirection }) {
  ...
  return <span className="ml-2 text-[#702963]">{direction === 'asc' ? '↑' : '↓'}</span>;
}

export function ClientTable({ clients, sortField, sortDirection, onSort, translations: t }: ClientTableProps) {
  if (clients.length === 0) {
    return (
    ...
    <div className="glass-card overflow-hidden">
      <div className="overflow-x-auto">
        <table className="data-table w-full">
          <thead>
            <tr>
              <th onClick={() => onSort('name')} className="cursor-pointer hover:bg-[#F3EFF5] select-none">
                {t.name}<SortIndicator field="name" active={sortField} direction={sortDirection} />
              </th>
              <th>{t.type}</th>
              <th>Status</th>
              <th onClick={() => onSort('aum')} className="cursor-pointer hover:bg-[#F3EFF5] select-none">
                {t.aum}<SortIndicator field="aum" active={sortField} direction={sortDirection} />
              </th>
              <th>Avg Quarterly AUM</th>
              <th>Commission Tier</th>
              <th onClick={() => onSort('commission')} className="cursor-pointer hover:bg-[#F3EFF5] select-none">
                Quarterly Commission<SortIndicator field="commission" active={sortField} direction={sortDirection} />
              </th>
              <th onClick={() => onSort('lastActivity')} className="cursor-pointer hover:bg-[#F3EFF5] select-none">
                {t.lastActivity}<SortIndicator field="lastActivity" active={sortField} direction={sortDirection} />
              </th>
            </tr>
          </thead>
          <tbody>
