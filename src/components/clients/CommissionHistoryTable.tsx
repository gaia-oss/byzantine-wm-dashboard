...
export function CommissionHistoryTable({ payouts }: CommissionHistoryTableProps) {
  return (
    <div className="glass-card overflow-hidden">
      ...
      {payouts.length > 0 ? (
            ...
            <tbody>
              {payouts.map((payout, index) => (
                ...
                >
                  ...
                  <td className="font-medium text-[#702963]">{formatCurrency(payout.amount)}</td>
                  <td>
                    <span className={`text-xs px-2 py-1 rounded font-medium ${STATUS_CLASSES[payout.status] || ''}`}>
        ...
        <div className="p-8 text-center text-[#9B8FA0]">No commission history found</div>
      )}
    </div>
  );
}
